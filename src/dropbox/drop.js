...
const REDIRECT_URI = 'http://localhost:5173/data.html';
...
async function handleOAuthReturn(accessToken) {
  try {
    ...
    if (window.connectAndSaveDropboxAccount) {
      const success = await window.connectAndSaveDropboxAccount(dropboxAuthData);
      if (!success) {
        showDropboxNotification('Failed to save account to database', 'error');
        return;
      }
    }
...
function removeDropboxAccount(accountId) {
  if (confirm('Are you sure you want to remove this account?')) {
    ...
    if (window.removeAccountFromSupabase) {
      window.removeAccountFromSupabase('dropbox', accountId);
    }
...
window.aaccounts = aaccounts;
window.currentdAccountId = currentdAccountId;
window.addAccountBtn = addAccountBtn;
window.renderAccounts = renderAccounts;
window.switchDropboxAccount = switchDropboxAccount;
window.removeDropboxAccount = removeDropboxAccount;